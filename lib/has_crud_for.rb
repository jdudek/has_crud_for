...
module HasCrudFor
  ...
  def define_crud_methods(models, name, methods = [])
    ...
    mixin.send :define_method, "find_#{name}" do |id|
      send(models).find(id)
    end if methods.include?(:find)

    mixin.send :define_method, "build_#{name}" do |hash = {}|
      send(models).build(hash)
    end if methods.include?(:build)

    mixin.send :define_method, "create_#{name}" do |hash = {}|
    ...
    mixin.send :define_method, "update_#{name}" do |id, hash|
      send(models).update(id, hash)
    end if methods.include?(:update)

    mixin.send :define_method, "destroy_#{name}" do |id|
  ...
  def define_crud_through_methods(models, name, parent, methods = [])
    ...
    mixin.send :define_method, "find_#{name}" do |parent_id, id|
      send("find_#{parent}", parent_id).send("find_#{model_name}", id)
    end if methods.include?(:find)

    mixin.send :define_method, "build_#{name}" do |parent_id, hash = {}|
      send("find_#{parent}", parent_id).send("build_#{model_name}", hash)
    end if methods.include?(:build)

    mixin.send :define_method, "create_#{name}" do |parent_id, hash = {}|
    ...
    mixin.send :define_method, "update_#{name}" do |parent_id, id, hash|
      send("find_#{parent}", parent_id).send("update_#{model_name}", id, hash)
    end if methods.include?(:update)

    mixin.send :define_method, "destroy_#{name}" do |parent_id, id|
